...
with pm.Model() as logistic_model:
    ...
    # Train Bayesian Logistic Regression (BLR)
    pm.glm.GLM.from_formula('income_more_50K ~ sex + age + age_squared + educ + hours', data=data, family=pm.glm.families.Binomial())
    map_estimate = pm.find_MAP()
    print(map_estimate)
    sampler = pm.Metropolis()
    # MCMC for sampling from the posterior from previous BLR
    trace = pm.sample(draws=400, step=sampler, chains=1, cores=4)
...
STUDENT_TOKEN = 'MAXdEACaZMHB9Lu4'
grader.status()
...
with pm.Model() as logistic_model:
    ...
    #### YOUR CODE HERE ####
    data['age_squared'] = np.square(data['age'])
    pm.glm.GLM.from_formula('income_more_50K ~ sex + age + educ + hours + age_squared', data=data, family='binomial')
    ### END OF YOUR CODE ###
    step = pm.Metropolis()
